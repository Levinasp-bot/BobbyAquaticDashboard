...
def show_dashboard(daily_profit_1, hw_forecast_future_1, daily_profit_2, hw_forecast_future_2, forecast_horizon=12, key_suffix=''):
    ...
    with col2:
        st.subheader('Peramalan Laba')

        # Adjust the x-axis based on the selected year range
        start_date = daily_profit_1.index.min() if daily_profit_1 is not None else daily_profit_2.index.min()
        end_date = daily_profit_1.index.max() if daily_profit_1 is not None else daily_profit_2.index.max()

        filtered_forecast_x = pd.date_range(start=start_date, end=end_date, freq='W').to_list()

        fig = go.Figure()

        # Plot historical data for both branches
        if daily_profit_1 is not None:
            fig.add_trace(go.Scatter(x=daily_profit_1.index, y=daily_profit_1['LABA'], mode='lines+markers', name='Profit Cabang 1', line=dict(color='blue')))
        if daily_profit_2 is not None:
            fig.add_trace(go.Scatter(x=daily_profit_2.index, y=daily_profit_2['LABA'], mode='lines+markers', name='Profit Cabang 2', line=dict(color='orange')))

        # Plot forecast data for both branches
        if hw_forecast_future_1 is not None:
            fig.add_trace(go.Scatter(x=filtered_forecast_x, y=hw_forecast_future_1, mode='lines+markers', name='Prediksi Cabang 1', line=dict(color='blue', dash='dash')))
        if hw_forecast_future_2 is not None:
            fig.add_trace(go.Scatter(x=filtered_forecast_x, y=hw_forecast_future_2, mode='lines+markers', name='Prediksi Cabang 2', line=dict(color='orange', dash='dash')))

        fig.update_layout(title='Perbandingan Laba Cabang 1 dan 2', xaxis_title='Tanggal', yaxis_title='Laba', xaxis_rangeslider_visible=True)
        st.plotly_chart(fig)
