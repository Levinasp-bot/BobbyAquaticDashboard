...
def show_dashboard(daily_profit_1, hw_forecast_future_1, daily_profit_2, hw_forecast_future_2, forecast_horizon=12, key_suffix=''):
    ...
    with col2:
        # Line graph showing the historical and forecasted data
        fig = go.Figure()

        if daily_profit_1 is not None:
            fig.add_trace(go.Scatter(x=daily_profit_1.index, y=daily_profit_1['LABA'], mode='lines', name='Laba Cabang 1'))
            fig.add_trace(go.Scatter(x=hw_forecast_future_1.index, y=hw_forecast_future_1, mode='lines', name='Prediksi Laba Cabang 1', line=dict(dash='dash')))

        if daily_profit_2 is not None:
            fig.add_trace(go.Scatter(x=daily_profit_2.index, y=daily_profit_2['LABA'], mode='lines', name='Laba Cabang 2'))
            fig.add_trace(go.Scatter(x=hw_forecast_future_2.index, y=hw_forecast_future_2, mode='lines', name='Prediksi Laba Cabang 2', line=dict(dash='dash')))
